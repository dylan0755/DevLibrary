...
public class GifDecoder {
    ...
    protected boolean transparency = true;//是否使用透明色
    ...
    protected void readGraphicControlExt() {
        ...
        transparency = (packed & 1) != 0;//从packed中解析出透明色标志
        int delayInHundredthsOfASecond = readShort();//读取延迟时间(毫秒)
        if (delayInHundredthsOfASecond < 2) {//有些是-1，显然是不行的，给个默认值吧
            delayInHundredthsOfASecond = 10;
        }
        delay = delayInHundredthsOfASecond * 10;
        transIndex = read();//读取透明色索引
        read();//按读取顺序，此处为标识块终结(Block Terminator)
    }
...
//            String path= Environment.getExternalStorageDirectory().toString() + File.separator + "gpt_robot.gif";
...
//                            GifDecoder.GifFrame[] gifFrames = gifDecoder.getFrames();
//                            Logger.d("gifFrames="+gifFrames.length);
//                            while(true){
//                                for (GifDecoder.GifFrame gifFrame: gifFrames) {
//                                    Logger.d("gifFrame.delay="+gifFrame.delay);
////                                    b.ivAiRobot.post(new Runnable() {
////                                        @Override
////                                        public void run() {
////                                            b.ivAiRobot.setImageBitmap(gifFrame.image);
////                                        }
////                                    });
//                                }
//                            }
