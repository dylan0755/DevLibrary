...
/**
 * 音频的录制多一个encode的步骤，而视频因为MediaCodec 中的Surface 有数据的时候会自动完成编码
 */
	...
	android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
        ...
        while (true) {
            synchronized (mLock) {
                ...
                localRequestDrain = (mRequestDrain > 0);//判断是否有数据进行绘制
...
//        	throw new NullPointerException("muxer is unexpectedly null");
            ...
            while (mIsCapturing) {
                ...
                if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {//循环几次没有数据说明缓冲池里已经没有数据，跳出循环
