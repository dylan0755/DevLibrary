...
public class MimeTypeFile {
    ...
    public static final int FILE_TYPE_MS_POWERPOINT_X = 108;
    public static final int FILE_TYPE_MS_WORD_X = 109;
    public static final int FILE_TYPE_MS_EXCEL_X  = 110;
    ...
    static {
        ...
        addFileType("DOCX",FILE_TYPE_MS_WORD_X,"application/vnd.openxmlformats-officedocument.wordprocessingml.document");
        addFileType("XLS", FILE_TYPE_MS_EXCEL, "application/vnd.ms-excel");
        addFileType("XLSX",FILE_TYPE_MS_EXCEL_X,"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        addFileType("PPT", FILE_TYPE_MS_POWERPOINT, "application/mspowerpoint");
        addFileType("PPTX",FILE_TYPE_MS_POWERPOINT_X,"application/vnd.openxmlformats-officedocument.presentationml.presentation");
     ...
     */
    public  boolean isDrmFileType(int fileType) {
    ...
    public static Intent getOpenFileIntent(Context context,String path) {
        MimeTypeFile.MediaFileType mediaFileType = MimeTypeFile.getFileType(path);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            uri = FileProvider.getUriForFile(context, AndroidManifestUtils.getFileProviderAuthority(context), new File(path));
            intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);//给目标文件临时授权，必需添加
        } else {
            uri = Uri.fromFile(new File(path));
        }
        if (mediaFileType != null) {
            intent.setDataAndType(uri, mediaFileType.mimeType);
        }else{
