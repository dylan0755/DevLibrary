...
public class WebViewImageSelector {
    ...
    public void setUploadFile(ValueCallback<Uri> uploadMsg){
    ...
    public  void onActivityResult(Context context,int requestCode, int resultCode, Intent data) {
        if(requestCode==REQUEST_UPLOAD_FILE_CODE)
        {
            if (null == mUploadFile && null == filePathCallback) return;
            ...
            if (filePathCallback != null) {
                ...
                if (result != null) {
                    String path = getPath(context.getApplicationContext(),result);
                    Uri uri = Uri.fromFile(new File(path));
                    mUploadFile.onReceiveValue(uri);
                } else {
                    mUploadFile.onReceiveValue(imageUri);
                }
                mUploadFile = null;
            }
        }
    ...
    public  void select(Activity activity){
        ...
        i.setType("image/*");
        Intent chooserIntent = Intent.createChooser(i,"上传图片");
        chooserIntent.putExtra(Intent.EXTRA_INITIAL_INTENTS, cameraIntents.toArray(new Parcelable[]{}));
        activity.startActivityForResult(chooserIntent,  REQUEST_UPLOAD_FILE_CODE);
    }
